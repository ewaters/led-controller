...
Animation.prototype = {
			...
			_.forEach(config.playback, function (playback) {
				...
				_.forEach(animation.frames, function (frame, frameIndex) {
					...
					if (func instanceof Error) {
						err = "Failed to compile playback " + playback.id + " frame " + frameIndex + ": " + func.message;
						return false;
					}
	...
	renderPlayback: function (playback) {
		...
		var strandsColors = this.forEachLayoutLocation(layout, renderFunc, false, {
			wantPixelColors: false,
	...
	renderStaticFrame: function (frame, frameIndex, animation, layout) {
		var func = this.getRenderFramePixelFunc(frame, frameIndex, animation, layout);
	...
	getRenderFramePixelFunc: function (frame, frameIndex, animation, layout) {
		...
		if (this.isTransitionFrame(frame)) {
			...
			if (prevFrame === undefined || nextFrame === undefined)
				return new Error("frame " + frameIndex + " can't be a transition without a frame before and after");
			if (this.isTransitionFrame(prevFrame) || this.isTransitionFrame(nextFrame))
				return new Error("frame " + frameIndex + " can't transition from/to another transition")

			return function (c) {
				if (c.stash.prev === undefined) {
					c.stash.prev = self.renderStaticFrame(prevFrame, frameIndex - 1, animation, layout);
					c.stash.next = self.renderStaticFrame(nextFrame, frameIndex + 1, animation, layout);
				}
				var from = c.stash.prev[c.pixelIndex],
					to   = c.stash.next[c.pixelIndex];
				//console.info("From " + from + " to " + to + " for index " + c.pixelIndex + " at " + context.percent);
				var tween = new Color("RGB", from).tween(c.percent, new Color("RGB", to));
	...
	forEachLayoutLocation: function (layout, func, wantPixelColors, superContext) {
		var pixelColors = [];
