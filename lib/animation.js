...
Animation.prototype = {
	...
	render: function (cb) {
		...
		async.mapSeries(this.config.playback, function (playback, cb) {
			...
			var playbackResult = self.renderPlayback(playback);
			...
			if (! playback.concurrentWithNext) completePlayback = true;
			return cb();
		}, function () {
	...
	renderPlayback: function (playback) {
		...
		var renderedFrame = renderFunc(renderContext);
	...
	getRenderFrameFunc: function (opt, cb) {
		...
		if (this.isTransitionFrame(frame)) {
			...
			if (frame.dissolve) {
				return cb(null, function (c) {
					var fromColors = c.playback.compiledFrames[ frameIndex - 1 ](_.extend({ percent: 1 }, c)),
						toColors   = c.playback.compiledFrames[ frameIndex + 1 ](_.extend({ percent: 0 }, c));
					var result = {};
		...
		if (frame.fill) {
			...
			return cb(null, function () { return result; });
		}
