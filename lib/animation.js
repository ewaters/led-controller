...
Animation.prototype = {
	...
	renderPlayback: function (playback) {
		...
		var strandsColors = this.forEachLayoutLocation(layout, renderFunc, false, renderContext);
	...
	getRenderFramePixelFunc: function (frame, frameIndex, animation, layout) {
		...
		if (this.isTransitionFrame(frame)) {
			...
			return function (c) {
				var from = c.playback.compiledFrames[ frameIndex - 1 ](c),
					to   = c.playback.compiledFrames[ frameIndex + 1 ](c);
