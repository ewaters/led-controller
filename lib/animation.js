...
Animation.prototype = {
	...
	render: function () {
		...
		for (var i = this.renderContext.playbackIndex; i < this.config.playback.length; i++) {
			var playback       = this.config.playback[i],
				playbackResult = this.renderPlayback(playback);
			...
			if (! playback.concurrentWithNext) break;
		}
