...
Animation.prototype = {
	...
	validateConfig: function (config) {
		...
		for (;;) {
			...
			// Validate animations by rendering all frames.
	...
	renderPlayback: function (playback) {
		...
		var renderFunc = this.getRenderFramePixelFunc(curFrame, context.curFrameIndex, animation, layout);
	...
	getRenderFramePixelFunc: function (frame, frameIndex, animation, layout) {
		if (this.isTransitionFrame(frame)) {
			...
			var prevPixelColors = this.renderStaticFrame(prevFrame, frameIndex - 1, animation, layout),
				nextPixelColors = this.renderStaticFrame(nextFrame, frameIndex + 1, animation, layout);
