...
Animation.prototype = {
	...
	validateConfig: function (config) {
		...
		var err = this._validateConfig(config);
		if (err !== undefined) return new Error("Invalid config: " + err);
	},
	...
	getRenderFramePixelFunc: function (frame, frameIndex, animation, layout) {
		...
		if (frame.fill) {
			...
			for (var i = 0; i < frame.gradient.length; i++) {
				...
				var color = new Color(animation.colorspace, frame.gradient[i].color);
				if (color instanceof Error) return color;
				item.color = color;
			}
			return function () { return frame.gradient[0].color; };
		}
		else {
