...
Playback.prototype = {
	...
	redisConnectAnd: function (func, cb) {
		...
		func(client, function (err, result) {
			client.quit([], function () { if (cb) cb(err, result); });
		});
	},
	redisHeaderSearch: function (headers) {
		var self = this;
		return function (client, next) {
			client.lrange([ self.redisPrefix + "headers", 0, -1 ], function (err, value) {
				...
				var result = [];
				_.forEach(value, function (jsonItem) {
	...
	redisFetchOne: function (headers) {
		var self = this;
		return function (client, next) {
			async.waterfall([
				function (cb) {
					self.redisHeaderSearch(headers)(client, cb);
				},
				function (results, cb) {
					if (results.length === 0) return cb("No matching data");
					if (results.length > 1) return cb("Ambiguous target; matched " + results.length + " results");
					client.get(self.redisPrefix + "config:" + stringify(results[0]), cb);
				},
			], next);
		};
	},
	redisFetchMutateStore: function (headers, mutateFunc) {
		var self = this;
		var key;
		return function (client, next) {
			async.waterfall([
				...
				function (results, cb) {
					if (results.length === 0) return cb("No matching data");
					if (results.length > 1) return cb("Ambiguous target; matched " + results.length + " results");
					key = self.redisPrefix + "config:" + stringify(results[0]);
					client.get(key, cb);
				},
				function (data, cb) {
				...
				function (item, cb) {
					//console.info("Setting " + key + " to " + JSON.stringify(item));
					client.set(key, JSON.stringify(item), cb);
				},
			], next);
		};
	},
	...
	apiStore: function (req, res) {
		...
		this.redisConnectAnd(function (client, close) {
			async.parallel([
				...
				function (cb) {
					client.set(self.redisPrefix + "config:" + stringify(headers), JSON.stringify(body), cb);
				}
			], function (err) {
				if (err) {
	...
	apiSearch: function (req, res) {
		this.redisConnectAnd(
			this.redisHeaderSearch(req.body),
	...
	apiMarkPlayed: function (req, res) {
		var self = this;
		this.redisConnectAnd(
			this.redisFetchMutateStore(req.body, function (item, cb) {
				item.played_last = Math.floor(new Date().valueOf() / 1000);
				item.played_count = item.played_count === undefined ? 1 : item.played_count + 1;
				cb(null, item);
			}),
			function (err, value) {
				if (err) return res.send(500, err);
