...
function Color (space, value) {
	...
	if (space === "RGB") {
		if (!_.isNumber(value) || value < 0 || value > 0xffffff) {
			return new Error("Invalid RGB color value '" + JSON.stringify(value) + "'");
		}
		var b = value % 256;
		value -= b;
		value = value >> 8;
...
Color.prototype = {
	...
	magick: function (accuracy) {
		if (accuracy === undefined) accuracy = 0;
		var colorValues = [];
		[ "red", "green", "blue" ].forEach(function (chan) {
			colorValues.push( (this[chan]() * 100).toFixed(accuracy) + "%" );
		}, this);
